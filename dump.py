...
DUMP FB BANG ✓
THE IS SCRIPT FREE✓
SCRIPT CUMAN NGEDUMP BUKA CRACK YA ANJ✓
KONTAK WANGSAP +62 881-0100-54926✓
YOUTOBE AING » Yatzz Emz✓
FEBIH AING »  Ęmpri Ñuub✓
________________________________________________
\t{garis} author by : Yatzz-XD""")              
...
def login():
	...
	if cukuf in ["help","Help","HELP"]:
		print("")
		x=f"{P2}whatsapp admin *--> {H2}+62 881-0100-54926 {P2}harap chat klo ada kepentingan yang mau disampaikan ke author dmf\nini klo gak bisa diarahin ke whastapp admin yakk"
		vprint(panel(x,style=f"{H3}"))
		print("")
		x=f"{P2}sedang diarahkan ke whastapp author"
		vprint(panel(x,style=f"{H3}"))
		os.system('xdg-open https://wa.me/++62881010054926?text=bang+cara+pake+script+abang+kek+mana?')
		...
		print("")
		x=f"{P2}whatsapp admin *--> {H2}+62881010054926 {P2}harap chat klo memang ada yang error\nini klo gak bisa diarahin ke whastapp admin yakk"
		vprint(panel(x,style=f"{H3}"))
		print("")
		x=f"{P2}sedang diarahkan ke whastapp author"
		vprint(panel(x,style=f"{H3}"))
		os.system('xdg-open https://wa.me/+62881010054926?text=bang+script+mu+itu+ada+yang+error!!')
		input(f" {P}[{H}•{P}] kembali")
		login()
	elif cukuf in ["0","00"]:
		exit()
